...
func (s *encoder) EncodeEntry(ent zapcore.Entry, fields []zapcore.Field) (*buffer.Buffer, error) {
	if ent.Stack != "" {
		// Load stacktrace limit from .env
		envStacktraceLimit, err := strconv.Atoi(os.Getenv("log_stacktrace_limit"))
		if err != nil || envStacktraceLimit < 0 {
			envStacktraceLimit = 0
		}

		stackTraceLimit := envStacktraceLimit * 2
		if stackTraceLimit > 0 {
			stacks := strings.Split(ent.Stack, "\n")
			if len(stacks) > stackTraceLimit {
				ent.Stack = strings.Join(stacks[:stackTraceLimit], "\n")
			}
		}
